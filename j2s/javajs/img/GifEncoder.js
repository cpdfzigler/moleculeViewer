...
if (ic != null) this.transparentColor = ic.intValue ();
...
var nColors = 0;
var ptTransparent = -1;
out : for (var pt = 0, row = 0, transparentRgb = -1; row < this.height; ++row) {
...
}}if ((nColors = this.addColor (colorVector, ciHash, rgb, nColors)) > 250) {
colorVector = this.ditherPixels ();
break out;
}}
}
ciHash = null;
...
Clazz.defineMethod (c$, "addColor", 
 function (colorVector, ciHash, rgb, nColors) {
...
}, "javajs.img.GifEncoder.ColorVector,java.util.Map,~N,~N");
Clazz.defineMethod (c$, "ditherPixels", 
 function () {
var colorVector = Clazz.innerTypeInstance (javajs.img.GifEncoder.ColorVector, this, null);
var ciHash =  new java.util.Hashtable ();
var nColors = 0;
var sb = javajs.img.GifEncoder.toByteARGB (this.pixels);
var w4 = this.width * 4;
var r1 = 25;
var r2 = 2 * r1 + 1;
...
for (var k = 0; k < 3; k++) {
var ci = 4 * (i * this.width + j) + k + 1;
var cc = sb[ci];
var rc = Math.round (Clazz.doubleToInt ((cc + r1) / r2)) * r2;
var err = cc - rc;
sb[ci] = rc;
if (j + 1 < this.width) sb[ci + 4] += (err * 7) >> 4;
if (i + 1 == this.height) continue;
if (j > 0) sb[ci + w4 - 4] += (err * 3) >> 4;
sb[ci + w4] += (err * 5) >> 4;
if (j + 1 < this.width) sb[ci + w4 + 4] += (err * 1) >> 4;
}
}
}
this.pixels = javajs.img.GifEncoder.toIntARGB (sb);
for (var i = 0, pt = 0; i < this.height; ++i) {
for (var j = 0; j < this.width; ++j, pt++) {
nColors = this.addColor (colorVector, ciHash, this.pixels[pt], nColors);
}
}
...
c$.toByteARGB = Clazz.defineMethod (c$, "toByteARGB", 
function (argbs) {
var n = argbs.length * 4;
var iData =  Clazz.newIntArray (n, 0);
...
this.putByte (0xf9);
this.putByte (4);
this.putByte ((this.transparentIndex == -1 ? 0 : 9) | (this.delayTime100ths > 0 ? 2 : 0));
...
function (a, b) {
this.index = b;
if (a >= 0 || a == this.b$["javajs.img.GifEncoder"].transparentColor) this.b$["javajs.img.GifEncoder"].transparentIndex = b;
}, "~N,~N");
